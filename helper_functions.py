...
def line_stretch(line, size): return scale(line, xfact=size / line.length, yfact=size / line.length,
                                           origin=start_point(line))
...
def are_colinear(p1, p2, p3): return p2.intersects(line_from_points(p1, p3)) and not (
            p1.equals(p2) or p1.equals(p3) or p2.equals(p3))


def find_colinear_points(points): return [points[i - 1] for i in range(len(points)) if
                                          are_colinear(points[i - 2], points[i - 1], points[i])]
...
def buffer_line_symmetric(line_segments, offset): return [parallel_line(line_segments, offset, 'left'),
                                                          parallel_line(line_segments, offset, 'right')]
...
def angular_bisectors(polygon): return [
    line_stretch(line_from_points(v, vertex_at_index(i, polygon.buffer(-1))), polygon.length) for i, v in
    enumerate(get_vertices(polygon))]
...
def vertices_in_origin(vertices, node_map_origin, node_map): return [
    vertex_from_node(get_nodes(vertex, node_map), node_map_origin) for vertex in vertices]


def line_in_origin(line, node_map_origin, node_map): return line_from_points(
    *vertices_in_origin(get_vertices(line), node_map_origin, node_map))


def line_from_nodes(node1, node2, node_map): return line_from_points(vertex_from_node(node1, node_map),
                                                                     vertex_from_node(node2, node_map))


def get_nodes(geom, node_map): return node_map[coords(geom)] if type(geom) == Point else [node_map[g] for g in
                                                                                          coords(geom)]
...
def edge_from_indices(index1, index2, polygon): return line_from_points(coords(polygon)[index1],
                                                                        coords(polygon)[index2])

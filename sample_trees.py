...
def otherLizardTree(T=nx.Graph(), node_map={}, scale_factor=SCALE_FACTOR):
    ...
    raw_points = Point(0, 0.507), Point(1.938, 0), Point(3.876, 0.507), Point(0, 3.507), Point(1.938, 4.000), Point(
        3.876, 3.507), Point(1.938, 2.000)
...
def antennaBeetleTree(T=nx.Graph(), node_map={}, scale_factor=SCALE_FACTOR):
    # TODO: do the actual NLopt for circle packing
    set_id = count()
    raw_points = Point(6, 2), Point(6, 0), Point(4, 0), Point(3, 2.5), Point(6, 6.12), Point(2, 0), Point(0, 0), Point(
        0, 2), Point(0, 6.12)
    pts = [Point(p.x * scale_factor, p.y * scale_factor) for p in raw_points]
    for p in pts:
        node_map[p.coords[0]] = next(set_id)
...
def beetleTree(T=nx.Graph(), node_map={}, scale_factor=SCALE_FACTOR):
    ...
    raw_points = Point(9.63, 29.26), Point(0, 26.56), Point(0, 14.56), Point(6.63, 0), Point(14.63, 11.71), Point(14.63,
                                                                                                                  21.94), Point(
        14.63, 25.94), Point(19.63, 29.26), Point(29.26, 26.56), Point(29.26, 14.56), Point(22.63, 0), Point(14.63,
                                                                                                             29.26)
    pts = [Point(p.x * scale_factor, p.y * scale_factor) for p in raw_points]
    for p in pts:
        node_map[p.coords[0]] = next(set_id)
